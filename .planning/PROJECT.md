...
## Current Milestone: v3.0 Self-Optimizing Graphs

**Goal:** Make bae's eval/optimization pipeline accessible and build the developer workflow for compiling and self-optimizing graphs.

**Target features:**
- Research eval ecosystem (DSPy, Braintrust, Promptfoo, RAGAS) and design bae's approach
- Eval DX that's super accessible — progressive complexity from zero-config to custom metrics
- CLI commands for the full lifecycle: create, run, eval, optimize, inspect
- Package scaffolding: `my_domain/graph.py` + `graph.md` with autogenerated mermaid diagrams
- Compiled artifacts from optimization runs stored in domain package (`.gitignore`-able)
- Developer workflow for "compile and self-optimize" as a natural part of development
