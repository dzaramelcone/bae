...
def _make_tool_wrapper(self, tool_name: str, method: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        if args and param_names:
            arg = str(args[0])  # Coerce to str for validation (tool args are string-originated)
            extra_kwargs = {}
...
1. `read("bae")` in a `<run>` block produces `[source] ◆ read(bae) -> str (N lines)` summary line, not a full execution panel
