...
must_haves:
  ...
  key_links:
    - from: "bae/repl/ai.py"
      ...
      via: "tool_summary metadata and final flag on response metadata"
      pattern: "tool_summary|final"
---
...
**1. New `_tool_summary` format.**
Replace `_tool_summary()` to produce: `◆ name(args) -> return_type (count_hint)`
...
Example outputs:
- `◆ read(bae.repl.ai) -> str (42 lines)`
- `◆ glob(bae.repl.*) -> str (5 matches)`
- `◆ write(bae.repl.new_mod) -> str`
...
Update `AI.__call__` tool call processing to unpack the new 3-tuple from `run_tool_calls`:
```python
for tag, output, is_error in tool_results:
    summary = _tool_summary(tag, output, is_error=is_error)
...
<success_criteria>
- `_tool_summary` produces `◆ name(args) -> type (hint)` format
