...
must_haves:
  truths:
    - "AI response containing a tool tag triggers translate_tool_calls before extract_executable"
    - "Translated code executes via async_exec in the REPL namespace"
    - "Execution output is fed back to AI with [Tool output] prefix"
    - "User sees tool_translated metadata type on the [py] channel write"
    - "System prompt teaches the AI all 5 terse tool tag formats"
    - "Tool call translation counts against max_eval_iters"
  artifacts:
    - path: "bae/repl/ai.py"
...
- `translate_tool_calls()` instead of `extract_executable()`
...
`pytest tests/repl/test_ai.py -v` — full suite green.
  ...
  <done>
Eval loop checks `translate_tool_calls()` before `extract_executable()`. Tool tag in response triggers translation -> async_exec -> feedback. Router writes use `tool_translated`/`tool_result` metadata. Tool calls count against max_eval_iters.
  </done>
</task>
...
- Tool tag in response triggers: translate -> async_exec -> [Tool output] feedback -> continue loop
- Router writes use `tool_translated` and `tool_result` metadata types (distinct from `ai_exec`/`ai_exec_result`)
...
- Tool calls count against `max_eval_iters`
