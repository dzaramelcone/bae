...
- `mark_done(self, task_id, changed_by='agent') -> dict`: Transition to 'done'. Check task_dependencies table — if task has blockers that are not done/cancelled, raise ValueError with blocking task IDs. For major tasks (minor=0, patch=0), validate body contains completion sections: `<outcome>`, `<confidence>`, `<retrospective>` before transitioning to done state. Raise ValueError if missing. If major task, self-verify (return info about whether minor subtasks are done). Log audit. If user_gated, return flag indicating user confirmation needed.
