...
must_haves:
  ...
  key_links:
    ...
    - from: "bae/repl/bash.py"
      to: "(nothing)"
      via: "No print() or print_formatted_text() calls remain -- returns raw data only"
...
<task type="auto">
  ...
  <verify>
    ...
    Verify bash.py contains no print() or print_formatted_text() calls (only return statements):
    ```bash
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in ('print', 'print_formatted_text'):
            print(f'FAIL: bare {node.func.id}() call at line {node.lineno}')
            sys.exit(1)
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr in ('print', 'print_formatted_text'):
            pass  # method calls are fine (e.g., logger.print)
    print('PASS: no print()/print_formatted_text() calls in bash.py')
    "
    ```
  ...
  <done>
    ...
    - bash.py returns raw data without printing -- verified by AST check: zero print()/print_formatted_text() calls
