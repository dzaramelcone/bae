...
must_haves:
  truths:
    ...
    - "Enter submits input, Shift+Enter inserts newline in all modes (Escape+Enter as terminal fallback)"
...
- PromptSession configured with:
  ...
  - `key_bindings`: KeyBindings instance with:
    ...
    - Register kitty keyboard protocol Shift+Enter: add `"\x1b[13;2u"` to prompt_toolkit's `ANSI_SEQUENCES` mapping as a custom `Keys("s-enter")` key, then bind `"s-enter"` to `insert_text("\n")`. This makes Shift+Enter work as newline on kitty-protocol terminals (Ghostty, kitty, iTerm2 CSI u mode) -- primary binding per user decision.
    - `escape` then `enter`: `insert_text("\n")` (newline -- terminal fallback for terminals that don't distinguish Shift+Enter from Enter)
- Main loop in `run()`:
...
- Enter submits, Shift+Enter inserts newline (Escape+Enter as terminal fallback); multiline editing works
