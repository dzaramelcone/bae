...
must_haves:
  truths:
    ...
    - "_cmd_run has no type injection code -- graph callables handle their own param flattening"
  artifacts:
...
<task type="auto">
  <name>Task 1: Add ANSI metadata to router.write calls and remove type injection from _cmd_run</name>
  <files>bae/repl/graph_commands.py</files>
  <action>
Three changes to graph_commands.py:

**1. Remove type injection block from _cmd_run** (lines 49-56): Delete the entire block that iterates `shell.namespace.values()` looking for `_param_types`. This is no longer needed — graph() callables now flatten BaseModel params directly. Remove:
```python
...
**2. _cmd_list ANSI metadata**: `shell.router.write("graph", _rich_to_ansi(table).rstrip("\n"), mode="GRAPH")`
   Change to: `shell.router.write("graph", _rich_to_ansi(table).rstrip("\n"), mode="GRAPH", metadata={"type": "ansi"})`

**3. _cmd_inspect ANSI metadata**: `shell.router.write("graph", _rich_to_ansi(text).rstrip("\n"), mode="GRAPH")`
...
**tests/repl/test_graph_commands.py**:
1. Remove `test_run_injects_param_types` from TestCmdRun -- type injection no longer exists.
2. Remove `TInput` and `TTypedStart` test nodes from module level (no longer needed).
3. Remove `typed_graph` from the shell fixture.
4. Add a test that verifies _cmd_list passes metadata with type "ansi" to router.write. Use the existing FakeShell/FakeRouter pattern. Create a graph run in the registry, call _cmd_list, and check that FakeRouter captured a write with `metadata={"type": "ansi"}` in the call args. Similarly for _cmd_inspect.
  </action>
