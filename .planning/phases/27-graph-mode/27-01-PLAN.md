...
must_haves:
  truths:
    ...
    - "The internal Graph instance is fully encapsulated inside the closure -- not exposed on the wrapper"
  artifacts:
    - path: "bae/graph.py"
...
8. Sets `wrapper._name = start.__name__` -- a display name string for task naming and inspect output (e.g., `ootd._name` returns `"IsTheUserGettingDressed"`). Do NOT store the Graph instance on the wrapper. The internal `Graph` object `g` must remain fully encapsulated inside the closure. The engine needs only a display name, not the Graph.
...
- `test_graph_factory_has_name`: `fn = graph(start=FactoryStart)` -- `fn._name == "FactoryStart"` (string display name, NOT a Graph object). Also verify `not hasattr(fn, '_graph')` to confirm the Graph is not leaked.
...
`uv run python -c "import inspect; from examples.ootd import ootd; print(inspect.signature(ootd))"` -- shows signature with user_info, user_message params.
  ...
  <done>
`graph()` factory in bae/graph.py creates async callables with typed signatures. Internal Graph fully encapsulated in closure -- only `_name` (display string) exposed on wrapper. Exported from bae package. examples/ootd.py uses `ootd = graph(start=IsTheUserGettingDressed)`. All existing + new tests pass.
  </done>
</task>
...
- Internal Graph fully encapsulated in closure -- only _name string exposed on wrapper
