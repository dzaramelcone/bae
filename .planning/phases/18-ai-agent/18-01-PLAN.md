...
3. **AI class:**
   - `__init__(self, *, lm: LM, router: ChannelRouter, namespace: dict, model: str = "anthropic:claude-sonnet-4-20250514")` -- stores references, initializes empty `_history: list = []`, sets `_agent: Agent | None = None` and `_model = model`. No Agent construction here (lazy init per Pitfall 1).
